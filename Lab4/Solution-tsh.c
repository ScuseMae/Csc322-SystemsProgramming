 ...
 * ID # - 
 *
 * Replace the tsh.c file in the shlab-handout
 * with this file (& rename to tsh.c)
 *
 */
...
void eval(char *cmdline) 
{
	...
	//forking and execing child process
	if ((pid = fork()) ==  0)
	{
	    setpgid(0,0);
...
void sigchld_handler(int sig) 
{
	...
	//if Exited, child terminated normally
	if (WIFEXITED(status))
        {   
            deletejob(jobs, pid);
            if (verbose)
	...
	//if signaled, child terminated from  signal
